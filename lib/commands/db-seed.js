...
seedUserDb = function(userDocs, callback) {
  var keys = _.keys(userDocs);
  var userId = keys[0]; // should only be _1_ key
  var docs = userDocs[userId];
  var userDbName = getUserDbName({userId: userId});
  var db = nano.db.use(userDbName);
  async.map(docs, db.insert, callback);
};
...
seedUserDbs = function (data, callback) {
  ...
  var seedDocs = function (cb) {
    async.map(data.USER_DOCS, seedUserDb, cb);
  };
