    ...
    exec('kanso push', function(err, res) {
      if (err) return logger.error({
        ...
        async.series([
          ...
          function (callback) { seedOtherDbs(data, callback); },
          function (callback) { replicateDrunkTank(data, callback); }
        ], function (err, res) {
          if (err) return logger.error({
            message: "Error seeding." + err,
            error: err
          });
          else logger.end();
        });
...
seedOtherDb = function(docs, callback) {
  var keys = _.keys(docs);
  var dbName = keys[0]; // should only be _1_ key
  var db = nano.db.use(dbName);
  async.map(docs[dbName], db.insert, callback);
};

seedOtherDbs = function (data, callback) {
  // add design docs for user DBs
  var seedUserDDocs = function (cb) {
    async.map(data.USERS, replicateUserDdoc, cb);
  };
  // add seed documents to each db
  var seedDocs = function (cb) {
    async.map(data.USER_DOCS, seedOtherDb, cb);
  };
  logger.info('Seeding user dbs...');
  // make sure validation is in place before seeding
  async.series([seedUserDDocs, seedDocs], callback);
};

replicateDrunkTankOne = function(userDoc, callback) {
  var userDbName = getUserDbName({userId: userDoc.user_id});
  var opts = { filter: 'userddoc/replicate_all' };
  nano.db.replicate(userDbName, 'drunk_tank', opts, callback);
};

replicateDrunkTank = function(data, callback) {
  logger.info('Replicating user dbs to drunk_tank...');
  async.map(data.USERS, replicateDrunkTankOne, callback);
};
