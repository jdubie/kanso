...
removeZipCodeDb = function(url, callback) {
  var dbUrl = urlParse(url);
  dbUrl.pathname = '';
  var urlString = urlFormat(dbUrl);
  var nano = require('nano')(urlString);
  ...
  nano.db.list(function(err, dbs) {
    if (err) {
      ...
      if (dbs.indexOf(dbName) > -1) {
        nano.db.destroy('zipcodes', callback)
      } else {
        callback();
      }
    }
...
loadZipcodeDb = function(url, callback) {
  // ensure DB is removed
  removeZipCodeDb(url, function(err, res) {
    ...
    var urlString = urlFormat(dbUrl);
    var cmd = "cd zipcodedb && kanso push " + urlString;
    exec(cmd, function(err, res) {
      // get the zipcodes DB
      var zipdb = require('nano')(urlString);

      // load all the zipcode documents
      var zipdata = fs.readFileSync('./zipcodedb/data/zipcodes.js').toString();
