...
loadUserDdocDb = function (url, callback) {
  var coffeePath = utils.abspath('./userdb/userddoc.coffee');
  cmd = "coffee -bc " + coffeePath;
  exec(cmd, function(err, res) {
    var path = utils.abspath('./userdb/userddoc.js');
    ...
    var dbName = 'userddocdb'
    nano.db.destroy(dbName, function(err, res) {
      // NOTE: will error if doesn't exist (but whatever)
      nano.db.create(dbName, function(err, res) {
        if (err) {
          callback({
            'message': "Error creating userddocdb:",
            'error': err
          });
        } else {
          userDdocDb = nano.db.use(dbName);
          userDdocDb.insert(ddoc, ddoc._id, function(err, res) {
            if (err) {
              callback({
                'message': "Error inserting design doc",
                'error': err
              });
            } else {
              callback(null);
