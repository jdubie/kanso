...
exports.run = function (settings, args) {
    ...
    pathExists(dir, function (exists) {
        ...
        kansorc.extend(settings, dir + '/.kansorc', function (err, settings) {
            ...
            ensureZipcodes(url, function(err, res) {
              if (err) {
                return logger.error(err);
              }
            });
...
ensureZipcodes = function(url, callback) {
  var dbUrl = urlParse(url);
  dbUrl.pathname = '';
  var urlString = urlFormat(dbUrl);
  var nano = require('nano')(urlString);
    ...
    if (err) {
      ...
      if (dbs.indexOf(dbName) === -1) {
        nano.db.create(dbName, function(err, res) {
          if (err) {
            callback({
              'message': "Error creating zipcodes database.",
              'error': err
            })
          } else {
            var zipdb = nano.db.use('zipcodes');
            var zipdata = fs.readFileSync('./data/zipcodes.js').toString();
