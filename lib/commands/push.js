...
exports.run = function (settings, args) {
    ...
    pathExists(dir, function (exists) {
        ...
        kansorc.extend(settings, dir + '/.kansorc', function (err, settings) {
            ...
            if (path.basename(dir) === 'lifeswap') {
              // drunk_tank db
              ensureDrunkTankPresent(url, function(err, res) {
                if (err) return logger.error(err);
                // after drunk_tank db b/c want to replicate to it
                loadUserDdocDb(url, function(err, res) {
                  if (err) return logger.error(err);
                });
              });
              ensureMapperPresent(url, function(err, res) {
                if (err) return logger.error(err);
              });
              // load the zipcodes database
              loadZipcodeDb(url, function(err, res) {
                if (err) return logger.error(err);
              });
            }
        });
...
loadUserDdocDb = function (url, callback) {
  var dbUrl = urlParse(url);
  dbUrl.pathname = '';
  var baseUrl = urlFormat(dbUrl);
  dbUrl.pathname = 'userddocdb';
  var urlString = urlFormat(dbUrl);
  var cmd = "cd userdb && kanso push " + urlString;
  logger.info('Pushing userdb ddoc');
  exec(cmd, function(err, res) {
    ...
    // secure the database so only admins can access/change it
    var security = {
      admins: {
        names: [],
        roles: ['_admin']
    ...
    var userDdocDb = nano.db.use('userddocdb')
    userDdocDb.insert(security, '_security', function (err,res) {
      if (err) {
        callback(err);
        return;
      }
      nano.db.list(function (err, dbs) {
        // replicate ddoc to one user db
        replicateUserDdoc = function (userDbName, callback) {
          if (!userDbName.match(/users_/) && userDbName !== 'drunk_tank') {
            callback();
            return;
          }
          logger.info('Replicating userddoc to: ' + userDbName);
          var opts = { 'doc_ids': [ '_design/userddoc'] };
          nano.db.replicate('userddocdb', userDbName, opts, callback);
        };

        async.map(dbs, replicateUserDdoc, function(err,res) {
          if (err) {
            callback(err);
          } else {
            callback()
            return logger.info('Replicated userddoc to all dbs');
          }
        });
      });
    });
...
ensureMapperPresent = function(url, callback) {
  var dbUrl = urlParse(url);
  dbUrl.pathname = '';
  var urlString = urlFormat(dbUrl);
  var nano = require('nano')(urlString);
    ...
    if (err) {
      ...
      if (dbs.indexOf(dbName) === -1) {
        nano.db.create(dbName, function(err, res) {
          if (err) {
            callback({
              'message': "Error creating mapper database.",
              'error': err
            })
          } else {
            callback(null);
...
ensureDrunkTankPresent = function(url, callback) {
  var dbUrl = urlParse(url);
  dbUrl.pathname = '';
  var urlString = urlFormat(dbUrl);
  var nano = require('nano')(urlString);

  var dbName = 'drunk_tank';
  nano.db.list(function(err, dbs) {
    if (err) {
      return callback({
        message:  "Error listing databases.",
    ...
    if (dbs.indexOf(dbName) !== -1) return callback(null);
    nano.db.create(dbName, function(err, res) {
      if (err) {
        return callback({
          message:  "Error creating drunk_tank database.",
          error:    err
        });
      }
      // secure the database so only admins can access/change it
      ...
      var drunkTank = nano.db.use(dbName)
      drunkTank.insert(security, '_security', function (err,res) {
...
zipcodeDbExists = function(url, callback) {
  var dbUrl = urlParse(url);
  dbUrl.pathname = '';
  var baseUrl = urlFormat(dbUrl);
  var nano = require('nano')(baseUrl);
  ...
  nano.db.list(function(err, dbs) {
    if (err) {
      ...
      if (dbs.indexOf(dbName) > -1) callback(null, true);
...
loadZipcodeDb = function(url, callback) {
  // determine whether zipcode DB exists
  // (only load zipcode data if it does not exist)
  zipcodeDbExists(url, function(err, res) {
    if (err) callback(err, res);
    else {
      var zipDbExists = res;

      // push design doc and upload zipcode docs
      var dbUrl = urlParse(url);
      dbUrl.pathname = '';
      var baseUrl = urlFormat(dbUrl);
      dbUrl.pathname = 'zipcodes';
      var urlString = urlFormat(dbUrl);
      var cmd = "cd zipcodes && kanso push " + urlString;
      exec(cmd, function(err, res) {
        if (err) callback({
          'message': 'Error pushing zipcode DB',
           'error': err
        });
        else if (zipDbExists) callback(err, res);
        // upload the zipcode data
        else {
          // get the zipcodes DB
          var zipdb = require('nano')(urlString);
          // load all the zipcode documents
          var zipdata = fs.readFileSync('./zipcodes/data/zipcodes.js').toString();
          zipdata = eval(zipdata);
          var zipDocs = {'docs': zipdata};
          zipdb.bulk(zipDocs, function(err, res) {
            if (err) {
              callback({
                'message': "Error bulk uploading to zipcodes DB.",
                'error': err
              });
            }
            else {
              ...
              zipdb.insert(security, '_security', callback);
            }
          });
        }
      });
